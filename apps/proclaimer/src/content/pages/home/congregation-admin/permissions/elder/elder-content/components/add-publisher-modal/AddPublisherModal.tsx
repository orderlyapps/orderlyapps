...
import { elderPermissionCollection } from "@amodeo/proclaimer/feature/permission";
...
export function AddPublisherModal({ is_open, on_dismiss }: AddPublisherModalProps) {
  ...
  const { handleAdd } = useAddPermission({
    ...
    on_insert: (auth_user_id, congregation_id) =>
      elderPermissionCollection.insert({ auth_user_id, congregation_id, can_edit: true }),
    on_update: (key) =>
