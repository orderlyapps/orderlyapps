...
interface TemplateEditorProps {
  index: number;
  template: WeekendMeetingSmsTemplate;
  on_change: (template: WeekendMeetingSmsTemplate) => void;
}
...
export function TemplateEditor({ index, template, on_change }: TemplateEditorProps) {
  ...
  function handle_reset() {
    set_text(DEFAULT_WEEKEND_MEETING_SMS_TEMPLATE_TEXT);
    on_change({ name, text: DEFAULT_WEEKEND_MEETING_SMS_TEMPLATE_TEXT });
  }
  ...
  return (
    <>
      ...
      <IonItem lines="none" onMouseDown={(e) => e.preventDefault()}>
        {WEEKEND_MEETING_SMS_PLACEHOLDERS.map((ph) => (
